...
export default function DislocationBoard({ entities, onSelect }: Props) {
  ...
  return (
    <div className="bg-[#0B0E11]/40 border border-[#2D333B] rounded shadow-xl backdrop-blur-md overflow-hidden">
      {/* Header */}
      <div className="bg-[#0B0E11] border-b border-[#2D333B] px-3 py-2 flex items-center justify-between select-none gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <Radar className="w-3.5 h-3.5 text-[#FF9900] animate-pulse shrink-0" />
          <h2 className="font-sans text-xs font-bold tracking-widest text-[#D1D4DC] truncate">
            DISLOCATION RADAR
          </h2>
          <span className="text-[9px] text-[#00FF66] font-mono font-bold flex items-center gap-1 shrink-0">
            <span className="w-1.5 h-1.5 rounded-full bg-[#00FF66] led-blink" /> LIVE
          </span>
        </div>
        <span className="font-mono text-[9px] text-[#D1D4DC]/40 shrink-0">
          {flagged.length} SIG · {gaps.length} GAPS
        </span>
      </div>
            ...
            {flagged.map((e) => {
              ...
              return (
                <button
                  ...
                  onClick={() => onSelect(e)}
                  title={d.rationale}
                  className="cursor-pointer text-left shrink-0 w-[170px] sm:w-[190px] bg-[#1C2128]/40 border rounded p-2 hover:bg-[#1C2128]/70 transition"
                  style={{ borderColor: `${c}55` }}
                >
                  <div className="flex items-center justify-between mb-1 gap-1">
                    <span className="font-mono text-[11px] font-bold text-[#FF9900] truncate">{e.ticker}</span>
                    <span className="font-mono text-[10px] text-white font-bold shrink-0">{e.value.toFixed(1)}%</span>
                  </div>
                  <div className="flex items-center gap-1 text-[10px] font-mono font-bold mb-1" style={{ color: c }}>
                    <Zap className="w-3 h-3 shrink-0" /> {d.label}
                  </div>
                  <div className="w-full h-1 bg-[#050608] rounded overflow-hidden">
                    <div className="h-full transition-all duration-500" style={{ width: `${Math.round(d.severity * 100)}%`, background: c }} />
                  </div>
                </button>
              );
            })}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {gaps.map((e) => {
              ...
              return (
                <div
                  key={`g-${e.id}`}
                  className="shrink-0 w-[190px] sm:w-[210px] bg-[#1C2128]/40 border border-[#2D333B] rounded p-2"
                >
                  <div className="flex items-center justify-between mb-1 gap-1">
                    <button onClick={() => onSelect(e)} className="cursor-pointer font-mono text-[11px] font-bold text-[#FF9900] truncate hover:underline">
                      {e.ticker}
                    </button>
                    <span className={`font-mono text-[11px] font-black shrink-0 ${pos ? "text-[#00FF66]" : "text-[#FF3B30]"}`}>
                      {pos ? "+" : ""}{g.gapPP.toFixed(1)}pp
                    </span>
                  </div>
                  <div className="font-mono text-[10px] text-[#D1D4DC]/70 flex items-center gap-2 flex-wrap">
                    <span>POLY <span className="text-white font-bold">{g.polymarket.toFixed(1)}%</span></span>
                    <span className="text-[#D1D4DC]/30">vs</span>
                    <span>KALSHI <span className="text-white font-bold">{(g.kalshi ?? 0).toFixed(1)}%</span></span>
                  </div>
                  {g.url && (
                    <a
                      href={g.url}
